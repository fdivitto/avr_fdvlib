...
public:
  void init(HardwareSPIMaster* spi)
  {
    m_spi = spi;
    init();
  }

  void init();  // warning: this doesn't assign m_spi, so you have to call init(spi) or init() but only after init(spi)
...
private:
  inline void setSS()     { m_spi->select(); };
  inline void resetSS()   { m_spi->deselect(); };
