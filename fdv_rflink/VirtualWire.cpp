...
#include "../fdv_generic/fdv_platform.h"
...
extern "C"
{
  ...
  void vw_set_tx_pin(Pin const* pin)
  {
    vw_tx_pin = pin;
  }
  
  // Set the pin number for input receiver data
  void vw_set_rx_pin(Pin const* pin)
  {
    vw_rx_pin = pin;
  }
  ...
  {
    unsigned long start = fdv::millis();
    
    while (!vw_rx_done && ((fdv::millis() - start) < milliseconds))
      ;
    return vw_rx_done;
  }
...
#endif
  {
    ...
    if (vw_tx_enabled && vw_tx_sample++ == 0)
    {
      ...
      {
        vw_tx_pin->write( vw_tx_buf[vw_tx_index] & (1 << vw_tx_bit++) );
        if (vw_tx_bit >= 6)
        {
	        vw_tx_bit = 0;
          vw_tx_index++;
